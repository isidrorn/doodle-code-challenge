...
the generic catch-all exception handler, not a meaningful error. `MeetingService.create()` tracks
...
against `docker-compose`'s Postgres anyway (bulk create, slot filtering, and the full
